...
class SnapFoodGraphDataModule(GraphDataModule):
    ...
    def __init__(self, config: Config, test_size=0.2, val_size=0.2, num_workers=2, drop_last=True, train_data_path='', test_data_path='', graphs_path='', batch_size = 32, device='cpu', shuffle = False,start_data_load=0, end_data_load=-1, graph_type: TextGraphType = TextGraphType.FULL, load_preprocessed_data = True, reweights={},removals=[], *args, **kwargs):
        ...
        super(SnapFoodGraphDataModule, self)\
    ...
    def load_labels(self):
        ...
        self.train_df.columns = ['comment', 'label', 'label_id']
        ...
        labels = self.df['label_id'][:self.end_data_load - self.start_data_load]
    ...
    def __get_co_occurrence_graph(self):
        ...
        return CoOccurrenceGraphConstructor(self.df['comment'][:self.end_data_load], path.join(self.graphs_path, 'co_occ'), self.config, load_preprocessed_data=True, naming_prepend='graph', start_data_load=self.start_data_load, end_data_load=self.end_data_load)
    
    def __get_sequential_graph(self):
        Path(path.join(self.graphs_path, 'seq_gen')).mkdir(parents=True, exist_ok=True)
        return SequentialGraphConstructor(self.df['comment'][:self.end_data_load], path.join(self.graphs_path, 'seq_gen'), self.config, load_preprocessed_data=True, naming_prepend='graph', start_data_load=self.start_data_load, end_data_load=self.end_data_load , use_general_node=True)
